...
%left	';'
%left	','
%right	'=' LPE LME LMLE LDVE LMDE LRSHE LLSHE LANDE LXORE LORE LSNDE
%right	'?' ':'
%left	LOROR
%left	LANDAND
%left	'|'
...
%token	LIF LINT LLONG LREGISTER LRETURN LSHORT LSIZEOF LUSED LCHANSET
...
%token	LRESTRICT LINLINE LCOTHREAD LCOPROC LALT
%%
prog:
|	prog xdecl
...
|	LSWITCH '@' stmnt
	{
		$$ = new(OSWITCH, procalts($3, 0), $3);
		$$ = new(OLIST, lastalt, $$);
	}
|	LSWITCH '@' '@' stmnt
	{
		$$ = new(OSWITCH, procalts($4, 1), $4);
		$$ = new(OLIST, lastalt, $$);
	}
|	LBREAK ';'
	{
...
|	expr LSNDE expr
	{
		if($3->op == ODOT || $3->op == ONAME)
			$$ = chanopaddr(OSEND, $1, $3);
		else {
			$$ = new(OAS, chanval(copynod($1), OSEND), $3);
			$$ = new(OCOMMA, $$, chanop(OSEND, $1));
		}
	}
