...
struct Channel {
	...
	uchar	*v;		/* Array of s values in the channel (malloc) */
};
...
void*	rtchancreate(int size, int elemsize, int bufsize);
...
int	rtthreadcreate(uint, int, void *fn, ...);
int	rtproccreate(uint, int, void *fn, ...);
