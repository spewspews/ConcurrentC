 ...
 */
static int
newthread(Proc *p, int argc, void *fn, void **argv, uint stacksize, char *name, int grp)
{
	int id;
	Thread *t;
	...
	_threadinitstack(t, argc, fn, argv);
	t->grp = grp;
	if(name)
...
{
	return newthread(_threadgetproc(), 1, f, &arg, stacksize, nil, threadgetgrp());
}
 ...
 */
Proc*
_newproc(int argc, void *fn, void **arg, uint stacksize, char *name, int grp, int rforkflag)
{
	Proc *p;
	...
	newthread(p, argc, fn, arg, stacksize, name, grp);
...
procrfork(void (*f)(void *), void *arg, uint stacksize, int rforkflag)
{
	...
	p = _threadgetproc();
	assert(p->newproc == nil);
	p->newproc = _newproc(1, f, &arg, stacksize, nil, p->thread->grp, rforkflag);
	id = p->newproc->threads.head->id;
	_sched();
	return id;
}
